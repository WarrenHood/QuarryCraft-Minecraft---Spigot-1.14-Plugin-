...
public class Quarry extends BukkitRunnable {
	...
	int miningDelay = 20;
	int counter = 0;
	String owner; // This should be changed to UUID
	boolean paused;
	boolean finished;
	...
	public Quarry(Chest centreChest, String owner) {
		...
		if (Main.doWGProtection)
			doWGProtection();
		if (Main.doGPProtection)
			doGPProtection();
	...
	public void trust(OfflinePlayer player) {
	...
	public boolean untrust(OfflinePlayer player) {
		return untrust(player.getUniqueId());
	}
	...
	public String getTrustedPlayersSerialized() {
		List<UUID> trustedPlayers = new ArrayList<UUID>(this.trustedPlayers); // Cloning to keep it thread safe
		String serialized = "";
		for (UUID player : trustedPlayers) {
			serialized += (serialized.length() == 0 ? "" : ":") + player.toString();
		}
		return serialized;
	}

	public void setTrustedPlayers(String[] trustedPlayers) {
		for (String playerUuid : trustedPlayers)
			this.trustedPlayers.add(UUID.fromString(playerUuid));
	}

	public boolean pistonAllowed(World w, int x, int y, int z) {
		if (world.getName().equals(w.getName()) && x >= minX - 15 && x <= maxX + 15 && z >= minZ - 15 && z <= maxZ + 15 && y >= centreChestLocation.getBlockY() - 16 && y <= centreChestLocation.getBlockY() + 16)
			return false;
		return true;
	}
	...
	public void buildPlatform() {
		...
		for (int i = 0; i < maxX - minX + 1; i++) {
			...
			if (platZ == minZ - 1 || platZ == maxZ + 1 || platX == minX - 1 || platX == maxX + 1 || (platZ >= centreChestLocation.getBlockZ() - 3 && platZ <= centreChestLocation.getBlockZ() + 3) || (platX >= centreChestLocation.getBlockX() - 3 && platX <= centreChestLocation.getBlockX() + 3)) {
				...
				// currentBlock.getType().equals(Material.WATER) ||
				// currentBlock.getType().equals(Material.LAVA) ||
				// currentBlock.getType().equals(Material.DIRT) ||
				// currentBlock.getType().equals(Material.GRASS) ||
				...
				// currentBlock.getType().equals(Material.GRASS) ||
				// currentBlock.getType().equals(Material.GRASS_BLOCK) ||
				// currentBlock.getType().equals(Material.STONE) ||
				// currentBlock.getType().equals(Material.COBBLESTONE)) {
				if (classicMode)
	...
	public void calculateUpgrades() {
		int emeraldBlockCount = 0; // Emeralds blocks increase the mining rate. 4 emerald blocks reduces mining
									// delay by 1 tick
		int diamondBlockCount = 0; // Diamond blocks increase the blocks mined at a time. 4 diamond blocks give 1
	...
	public String toggleEndermining() {
		...
		platformDone = false;
		if (classicMode)
			return Main.configurableMessages.miningModeToggled() + ": " + ChatColor.GREEN + Main.configurableMessages.classic();
	...
	public boolean hasFuel(float amount) {
		...
		for (int i = 0; i < centreInv.getSize(); i++) {
			...
			if (currentFuel != null)
				storedEnergy += currentFuel.getEnergyValue() * centreInv.getItem(i).getAmount();
			if (storedEnergy >= amount) {
				// Bukkit.broadcastMessage("Needs " + amount + "energy, has " + energySoFar);
				return true;
			}
	...
	public boolean consumeFuel(float energyToConsume) {
	...
	// TODO: Maybe add #tellTrusted if the owner is not online
	...
	public boolean canInteractAt(Location l, Player player) {
		if (player.hasPermission("quarrycraft.useall"))
			return true;
		if ((isOwner(player) || isTrusted(player)) && player.hasPermission("quarrycraft.use")) {
			return true;
		}
		if (l.getWorld().getName().equals(world.getName()) && l.getBlockX() >= minX - 1 && l.getBlockX() <= maxX + 1 && l.getBlockZ() >= minZ - 1 && l.getBlockZ() <= maxZ + 1)
			return false;
		return true;
	}
	...
	public boolean canBreak(Location l, Player p) {
		if ((isOwner(p) && p.hasPermission("quarrycraft.use")) || p.hasPermission("quarrycraft.useall")) {
			if (l.getWorld().getName().equals(world.getName()) && l.getBlockX() >= minX - 1 && l.getBlockX() <= maxX + 1 && l.getBlockZ() >= minZ - 1 && l.getBlockZ() <= maxZ + 1 && l.getBlockY() == centreChestLocation.getBlockY() - 1)
				return false;
			return true;
		}
		if (l.getWorld().getName().equals(world.getName()) && l.getBlockX() >= minX - 1 && l.getBlockX() <= maxX + 1 && l.getBlockZ() >= minZ - 1 && l.getBlockZ() <= maxZ + 1)
	...
	public boolean isIgnored(Material mat) {
		return Arrays.stream(ignored).anyMatch(mat::equals);
	}
	...
	public void mineNextBlock() {
		...
		if (classicMode) {
			...
			/*
			 * if(blockToMine.getLocation().getBlockY() == centreChestLocation.getBlockY()-1) { while(blockToMine.getType().equals(Material.BLACK_STAINED_GLASS) || blockToMine.getType().equals(Material.GREEN_STAINED_GLASS) || blockToMine.getType().equals(Material.CYAN_STAINED_GLASS)) { moveMiningCursor(); blockToMine = findNextBlock(); if(nextX == maxX && nextY == 0 && nextZ == maxZ)return; }
			 * 
			 * }
			 */
			...
			if (hasFuel(energyToUse)) {
				if (addMined(thisMaterial)) {
					...
					if (thisMaterial.equals(Material.DIRT) || thisMaterial.equals(Material.GRASS) || thisMaterial.equals(Material.GRASS_BLOCK))
			...
			/*
			 * if(blockToMine.getLocation().getBlockY() == centreChestLocation.getBlockY()-1) { while(blockToMine.getType().equals(Material.BLACK_STAINED_GLASS) || blockToMine.getType().equals(Material.GREEN_STAINED_GLASS) || blockToMine.getType().equals(Material.CYAN_STAINED_GLASS)) { moveMiningCursor(); blockToMine = findNextBlock(); thisMaterial = blockToMine.getType(); if(nextX == maxX && nextY == 0 && nextZ == maxZ) return; } }
			 */
